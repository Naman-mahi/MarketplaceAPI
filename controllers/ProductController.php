...
class ProductController
{
    ...
    public function getProductById($product_id)
    {
        $connection = getDbConnection();
        $stmt = $connection->prepare("
            SELECT p.product_id, p.dealer_id, p.category_id, p.product_name, p.product_description, p.is_featured, p.product_features, p.price, p.color, p.top_features, p.stand_out_features, p.created_at, p.updated_at,
                   pi.image_id, pi.image_url, pi.is_primary,
                   pa.pf_id, pa.category_id AS attribute_category_id, pa.pf_name,
                   pav.value,
                   pca.custom_attribute_id, pca.attribute_name, pca.attribute_value,
    ...
    ");
        ...
        $stmt = $connection->prepare("
        SELECT p.product_id, p.dealer_id, p.product_name, p.product_description, p.price,p.product_image, p.color, p.product_features, 
               pi.image_id, pi.image_url, pi.is_primary, pa.pf_id, pa.pf_name, pav.value, 
               pca.custom_attribute_id, pca.attribute_name, pca.attribute_value
        FROM products p
        LEFT JOIN product_images pi ON p.product_id = pi.product_id
        LEFT JOIN product_publish pub ON p.product_id = pub.product_id
        ...
        AND pub.website = 1
        AND p.product_id != ?
        AND (p.product_name LIKE ? OR p.product_description LIKE ? OR p.color LIKE ? OR p.price BETWEEN ? AND ? OR p.product_features LIKE ? OR p.brand_id = ?)
    ");
        ...
        $stmt->bind_param("iissddsss", $category_id, $product_id, $like_name, $like_description, $like_color, $price_min, $price_max, $like_features, $brand_id);
        $stmt->execute();
        $rows = $stmt->get_result();
        ...
        while ($row = $rows->fetch_assoc()) {
            // Structure the data in the same way as the single product fetch method
            ...
            if ($row['image_id'] && !in_array($row['image_id'], array_column($similarProducts[$similarProductId]['images'], 'image_id'))) {
                $similarProducts[$similarProductId]['images'][] = [
                    'image_id' => $row['image_id'],
                    'image_url' => IMAGES_URL . $row['image_url'],
                    'is_primary' => $row['is_primary'],
                ];
            ...
            // Add combined attributes if not already added
            if ($row['pf_id'] && !in_array($row['pf_id'], array_column($similarProducts[$similarProductId]['combined_attributes'], 'pf_id'))) {
                $similarProducts[$similarProductId]['combined_attributes'][] = [
                    'pf_id' => $row['pf_id'],
                    'pf_name' => $row['pf_name'],
                    'value' => $row['value'],
                ];
            ...
            // Add custom attributes if not already added
            if ($row['custom_attribute_id'] && !in_array($row['custom_attribute_id'], array_column($similarProducts[$similarProductId]['custom_attributes'], 'custom_attribute_id'))) {
                $similarProducts[$similarProductId]['custom_attributes'][] = [
                    'custom_attribute_id' => $row['custom_attribute_id'],
                    'attribute_name' => $row['attribute_name'],
                    'attribute_value' => $row['attribute_value'],
                ];
        ...
        if (!empty($similarProducts)) {
            return ['status' => 200, 'message' => 'Similar products fetched successfully', 'data' => array_values($similarProducts)];
        } else {
            return ['status' => 404, 'message' => 'No similar products found'];
        }
    }
